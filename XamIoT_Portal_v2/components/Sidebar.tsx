...
const NAV_LABELS: Record<Lang, Record<string, string>> = {
  ...
  es: {
    ...
    notifications: 'Reglas de alerta',
    alertes: 'Historial de alertas',
    support: 'Soporte',
    commandes: 'Mis pedidos',
    adresses: 'Mis direcciones',
    profile: 'Mi perfil',
  },
};
...
function NavLinks({ pathname, lang, onClose }: { pathname: string; lang: Lang; onClose?: () => void }) {
        ...
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          <aside className="relative z-10 flex flex-col w-72 max-w-[85vw] bg-white h-full shadow-xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <Logo />
              <button onClick={() => setMobileOpen(false)} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100 transition">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            </div>
            <NavLinks pathname={pathname} lang={lang} onClose={() => setMobileOpen(false)} />
            {footer}
          </aside>
        </div>
      )}
